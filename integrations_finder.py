...
class IntegrationsFinderGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Description
        desc = QLabel("Enter a SUSE Observability agent container tag or SHA to find the corresponding integrations source code")
        desc.setAlignment(Qt.AlignmentFlag.AlignCenter)
        desc.setWordWrap(True)
        layout.addWidget(desc)
        
        # Warning label for development versions (initially hidden)
        self.warning_label = QLabel("⚠️ WARNING: You are working with an unofficial/unreleased development version of the integrations")
        self.warning_label.setStyleSheet("color: red; font-weight: bold; background-color: #ffe6e6; padding: 8px; border: 2px solid red; border-radius: 4px;")
    ...
    def find_integrations(self):
        ...
        if not input_string:
            QMessageBox.warning(self, "Input Required", "Please enter a SUSE Observability agent SHA or container path.")
            return
